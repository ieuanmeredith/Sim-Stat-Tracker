...
@Component({
  selector: "dirt-road-comp",
  templateUrl: "views/components/app/dirt-road.html"
})
export class DirtRoadComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this.roadScope = this.settingsProvider.loadDirtRoadSettings();

    if (!this.dateFrom || !this.dateTo) {
      this.rawData = this.db.get("sessions").orderBy(x => x.session.start_time).value()
      .filter((session: object) => session["session"]["catid"] === 4) as IRaceSession[];
      ...
      this.rawData = this.db.get("sessions").orderBy(x => x.session.start_time).value()
        .filter(session => session["session"]["catid"] === 4
  ...
  public navigate (target: string) {
    ...
    this.settingsProvider.saveDirtRoadSettings(this.roadScope);
  }
...
@NgModule({
  ...
  declarations: [DirtRoadModule],
  bootstrap: [DirtRoadModule]
})
export class DirtRoadModule { }
