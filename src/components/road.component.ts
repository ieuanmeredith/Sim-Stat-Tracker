...
@Component({
  selector: "road-comp",
  templateUrl: "views/components/app/road.html"
})
export class RoadComponent implements OnInit {
  ...
  private userRaceResults: Array<{sessionId: any, result: IRow, subsessionId: any}>;
  ...
  public ngOnInit(): void {
    ...
    this.roadScope = this.settingsProvider.loadRoadSettings();

    if (!this.dateFrom || !this.dateTo) {
      this.rawData = this.db.get("sessions").orderBy(x => x.session.start_time).value()
      .filter((session: object) => session["session"]["catid"] === 2) as IRaceSession[];
      ...
      this.rawData = this.db.get("sessions").orderBy(x => x.session.start_time).value()
        .filter(session => session["session"]["catid"] === 2
      ...
      if (this.roadScope.overview) {
        this.roadScope.overview = false;
        setTimeout(() => { this.roadScope.overview = true; }, 50);
      }
    }
  ...
  public navigate (target: string) {
    ...
    this.settingsProvider.saveRoadSettings(this.roadScope);
  }

  public filterBySeason(season: string) {
...
@NgModule({
  ...
  declarations: [RoadModule],
  bootstrap: [RoadModule]
})
