...
@Component({
  selector: "oval-comp",
  templateUrl: "views/components/app/oval.html"
})
export class OvalComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this.roadScope = this.settingsProvider.loadOvalSettings();

    if (!this.dateFrom || !this.dateTo) {
      this.rawData = this.db.get("sessions").orderBy(x => x.session.start_time).value()
      .filter((session: object) => session["session"]["catid"] === 1) as IRaceSession[];
      ...
      this.rawData = this.db.get("sessions").orderBy(x => x.session.start_time).value()
        .filter(session => session["session"]["catid"] === 1
  ...
  public navigate (target: string) {
    ...
    this.settingsProvider.saveOvalSettings(this.roadScope);
  }
...
@NgModule({
  ...
  declarations: [OvalModule],
  bootstrap: [OvalModule]
})
